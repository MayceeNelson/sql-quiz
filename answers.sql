...
SELECT email  FROM salespeople WHERE 'Northwest' IN (region)  OR 'Southwest' IN (region);
...
SELECT email, first_name, last_name  FROM salespeople WHERE last_name LIKE 'M%'AND (region='Northwest'OR region = 'Southwest');